...
- [x] Properties panel for editing selected elements (name, color, etc.)
- [x] Vertex editing for placed rooms (click to adjust corners)
- [x] Snap doors to wall edges
- [x] Copy/paste elements

### Medium Priority

- [x] More decoration types in palette (bush, table, chair, bed, car)
- [x] Character types (shadow monster, player character, elderly residents, guard, npc) - all should be nameable
- [x] Route color picker
- [x] Loop toggle for patrol routes
- [x] Jump scare positions
- [x] Lots of different Icons for different item types (key, key-card, document, flashlight, battery, medkit)
- [x] Lighting indicator - add lighting level to the room and have it visually display so we can indicate low / high light areas

### Issues to fix:

- [x] When adding new elements we have to click an unoccupied position and drag it over other elements instead of directly layering
- [x] Backspace causes items to be deleted - this happens accidentally when trying to type inside its properties sidebar
